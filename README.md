...
- **接入任意 OpenAI 兼容 API**：设置面板填写 Base URL + API Key，可一键拉取模型列表，也可手动填写模型名。兼容 OpenAI、DeepSeek、Moonshot/Kimi、智谱 GLM、通义千问、OpenRouter，以及本地 Ollama / LM Studio。
- **一键分析**：每个计算器起盘后点击「✦ 一键分析」，内置专家级预设 Prompt 按固定结构输出全面解读。
- **追问聊天**：每个盘面下方都有聊天框，基于当前盘继续提问（"明年适合换工作吗？"），支持流式输出、停止生成、复制回答；重新起盘后下一条消息自动携带最新盘面。
